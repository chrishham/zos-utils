...
module.exports = (config) => {
  class ZosJob extends EventEmitter {
    constructor (jcl) {
      ...
      this.watchJobInterval = (config.watchJobInterval && config.watchJobInterval >= 1500) || 1500
        ...
        try {
          ...
          switch (jclType) {
            ...
            case 'hostFile':
              ...
              let stream = await this.ftp.get(this.jcl.source)
              stream.resume()
              jclToSubmit = await streamHandler(stream, this.encoding)
...
async function ftpDebug (data, self, subResolve, subReject) {
  ...
  if (/250-It is known to JES/.test(data)) {
    ...
    self.emit('job-id', self.id)
    setTimeout(() => ftpList(self, subResolve, subReject), 6000)
  }
}
